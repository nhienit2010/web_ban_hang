...
<?php
  ...
  if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['confirm_password']) 
    ...
    if ($password !== $confirm_password) {
        echo "<script>alert('Mật khẩu xác nhận không khớp!!'); window.location= '/register.php'</script>";
        exit();
    }

    if (!preg_match('/^[0-9]{10,}/', $phone)) {
        echo "<script>alert('Số điện thoại không đúng!!'); window.location= '/register.php'</script>";
        exit();
    }
    ...
    if($result->num_rows == 1) {    
        echo "<script>alert('Tên người dùng đã tồn tại!!'); window.location= '/register.php'</script>";
        exit();
    }
    else {
        ...
        $query->execute();
        echo "<script>alert('Đăng ký thành công!! Vui lòng đăng nhập nà!!'); window.location= '/login.php'</script>";
    }
  }
?>
